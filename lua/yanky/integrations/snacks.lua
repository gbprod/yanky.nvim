...
function M.pick()
  if not _G.Snacks then
    vim.notify("Snacks is not set up. Please install before using it.", vim.log.levels.ERROR)
    return
  end
  ...
  local function format_title(item)
    if item.filetype == nil then
      return regtype_to_text(item.regtype)
    end

    return regtype_to_text(item.regtype) .. " (" .. item.filetype .. ")"
  end
